...
async def generate_request_embed(**kwargs):
    ...
    embed.set_author(name=beatmap.creator,
                     icon_url=f"https://a.ppy.sh/{beatmap.creator_id}?.png")
    embed.set_footer(text="Status: Pending")

    embed.add_field(
        name="Map Name", value="`{0.artist} - {0.title}`".format(beatmap), inline=True)
    embed.add_field(name="Diff count",
                    value=f"`{len(beatmapset)}`", inline=True)
